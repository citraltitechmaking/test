...
def solve():
    ...
    # https://atcoder.jp/contests/tessoku-book/submissions/50092888
    ...
    class MaxFlowGraph:
        def __init__(self, n):
            self.n = n
            self.graph = [list() for i in range(self.n + 1)]

        def dfs(self, pos, goal, tmp_flow, graph, used):
            # print(pos, goal, tmp_flow, used)
            if pos == goal:
                return tmp_flow
            used[pos] = True
            for edge in graph[pos]:
                # 容量が 1 以上でかつ、まだ訪問していない頂点にのみ行く
                if edge.cap > 0 and not used[edge.to]:
                    flow = self.dfs(edge.to, goal, min(tmp_flow, edge.cap), graph, used)
                    if flow >= 1:
                        edge.cap -= flow
        ...
        def add_edge(self, src, dst, cap):
            self.graph[src].append(MaxFlowEdge(dst, cap, len(self.graph[dst])))
            self.graph[dst].append(MaxFlowEdge(src, 0, len(self.graph[src]) - 1))
            # for i, tmp in enumerate(self.graph):
            #     print(i, [(v.to, v.cap, v.rev) for v in tmp])
            # print()
        ...
        def execute(self, s, t):
            ...
            while True:
                used = [False] * (self.n + 1)
                tmp_flow = self.dfs(s, t, INF, self.graph, used)
                if tmp_flow > 0:
                    total_flow += tmp_flow
                else:
                    break
    ...
    maxflow_graph = MaxFlowGraph(n)
    for src, dst, cap in edges:
        maxflow_graph.add_edge(src, dst, cap)

    # execute(from, to)
    ans = maxflow_graph.execute(1, n)
    print(ans)
